...
class Examination
{
	...
	function __construct()
	{
		...
		$this->username = 'root';
		$this->password = '';
		$this->database = 'my_online_exam_system';
		$this->home_page = 'http://localhost/My_Online_Exam_System/';
	...
	function send_email($receiver_email, $subject, $body)
	{
		...
		$mail->From = 'informationcomputer815@gmail.com';

		$mail->FromName = 'informationcomputer815@gmail.com';
	...
	function admin_session_private()
	{
		...
		{
			$this->redirect('Admin_Login-page.php');
		}
	}
	...
	function Is_exam_is_not_started($online_exam_id)
	{
		...
		$this->query = "SELECT online_exam_datetime FROM online_exam_table 
	...
	{
		$this->query = "SELECT total_question FROM online_exam_table 
	...
	{
		$this->query = "SELECT online_exam_id FROM online_exam_table 
	...
	function Upload_file()
	{
		if(!empty($this->filedata['name']))
		{
			...
			$target_path = '../Student/upload/' . $new_name;
	...
	function user_session_private()
	{
		...
		{
			$this->redirect('User_Login-page.php');
		}
	}
	...
	function Fill_exam_list()
	{
		...
		$this->query = "SELECT oe.online_exam_id, oe.online_exam_title 
	...
	{
		$this->query = "SELECT * FROM user_exam_enroll_table 
	...
	// {
	// 	$this->query = "SELECT user_answer_option FROM user_exam_question_answer 
	...
	// {
	// 	$this->query = "SELECT marks_per_right_answer FROM online_exam_table 
	// 	WHERE online_exam_id = '".$exam_id."' 
	// 	";
	...
	// {
	// 	$this->query = "SELECT marks_per_wrong_answer FROM online_exam_table 
	// 	WHERE online_exam_id = '".$exam_id."' 
	// 	";
	...
	// {
	// 	$this->query = "SELECT answer_option FROM question_table 
	// 	WHERE question_id = '".$question_id."' 
	// 	";
	...
	{
		$this->query = "SELECT online_exam_status FROM online_exam_table 
		WHERE online_exam_id = '".$exam_id."' 
		";
		$result = $this->query_result();
		foreach($result as $row)
	...
	{
		$this->query = "SELECT attendance_status 
